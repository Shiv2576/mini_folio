...
function TechBadge({ skill }) {
  ...
  const handleMouseMove = (e) => {
    ...
    rafRef.current = requestAnimationFrame(() => {
      ...
      // Increased magnetic distance from 150 to 300
      const magneticRange = 300;

      if (distance < magneticRange) {
        const strength = Math.max(0, 1 - distance / magneticRange);
        // Slightly reduced multiplier to keep movement smooth with larger range
        setMagnetPos({
          x: distX * 0.12 * strength,
          y: distY * 0.12 * strength,
        });
      } else {
    ...
    <div ref={badgeRef} className="relative">
      <div
        ...
        style={{
          ...
          transition:
            magnetPos.x === 0 && magnetPos.y === 0 && !isHovered
              ? "transform 0.3s ease-out, border-color 0.2s ease, box-shadow 0.2s ease"
              : "border-color 0.2s ease, box-shadow 0.2s ease",
          willChange:
            magnetPos.x !== 0 || magnetPos.y !== 0 ? "transform" : "auto",
        }}
      >
        {/* Icon */}
